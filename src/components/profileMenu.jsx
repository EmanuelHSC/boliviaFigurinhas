  ...
  return (
    <>
      ...
      <Menu id="profile-menu" anchorEl={anchorEl} keepMounted open={Boolean(anchorEl)} onClose={handleClose} sx={{ minWidth: "30rem" }}>
        {user.isLoggedIn
          ? [
              <MenuItem key="logout" onClick={handleLogout} sx={{ minHeight: "4rem" }} style={{ fontSize: "1rem", color: "black" }}>
                Sair
              </MenuItem>,
              (!REQUIRE_ADMIN || user.isAdmin) && [
                <MenuItem key="adminPanel" sx={{ minHeight: "4rem" }} style={{ fontSize: "1rem", color: "black" }}>
                  <Link to="/admin" style={{ textDecoration: "none", color: "inherit" }}>
                    Painel Admin
                  </Link>
                </MenuItem>,
                <MenuItem key="userAdmin" sx={{ minHeight: "4rem" }} style={{ fontSize: "1rem", color: "black" }}>
